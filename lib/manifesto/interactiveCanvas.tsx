...
const LOOS_FONT_FAMILY = '"LoosExtended", sans-serif';

/**
 * Type interface for tracking individual star vector nodes.
 */
...
export default function InteractiveCanvas(): React.JSX.Element {
	...
	useEffect(() => {
		...
		canvas.addEventListener("touchstart", handleTouchStartRaw, {
			passive: false,
		});
		canvas.addEventListener("touchmove", handleTouchMoveRaw, {
			passive: false,
		});
	...
	useEffect(() => {
		...
		MANIFESTO_PARAGRAPHS.forEach((paragraph) => {
			...
			if (paragraph.type === "emphasis") {
				...
				fontSetting = `900 ${isMobile ? "26px" : "36px"} ${LOOS_FONT_FAMILY}`;
		...
		MANIFESTO_PARAGRAPHS.forEach((paragraph) => {
			if (paragraph.type === "questions") {
				...
				for (let i = 0; i < numChars; i++) {
					const currentSize = startSize + i * sizeStep;
					ctx.font = `500 ${currentSize}px ${LOOS_FONT_FAMILY}`;
		...
		stars.forEach((star) => {
			...
			const currentRenderColor =
				star.id === editingStarId ? "#ff4444" : star.color;
