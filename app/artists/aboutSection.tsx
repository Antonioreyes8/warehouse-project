...
export default function AboutSection({ profile, works }: AboutSectionProps) {
	...
	return (
		<section className={styles.aboutSection}>
			...
			{/* HOT TAKES */}
			{profile.hot_takes && Object.keys(profile.hot_takes).length > 0 && (
				<div className={styles.hotTakesContainer}>
					<h3>Hot Takes</h3>
					<div className={styles.hotTakesList}>
						{QUESTIONS.filter((q) => q.id in (profile.hot_takes ?? {})).map(
							(q) => (
								<div key={q.id} className={styles.hotTakeRow}>
									<p className={styles.hotTakeText}>{q.text}</p>
									<p className={styles.hotTakeResult}>
										{profile.hot_takes?.[q.id] ? "Agree" : "Disagree"}
									</p>
								</div>
							),
						)}
